...
1. Разделить строку на фразы.
2. Для каждой фразы подсчитать количество гласных букв.
3. Сравнить количество гласных букв во всех фразах.
4. Вывести соответствующее сообщение в зависимости от результата сравнения.

Мы будем использовать следующие шаги:
1. Разделим строку на фразы по пробелам.
2. Для каждой фразы подсчитаем количество гласных букв, используя генератор списков и функцию sum.
3. Сравним полученные количества гласных для каждой фразы.
4. Выведем результат.

Объяснение кода:

1. vowels = "аеёиоуыэюя" — строка с гласными буквами русского алфавита.
2. phrases = stroka.split() — разделяем исходную строку на фразы по пробелам.
3. Проверка на количество фраз: если их меньше или равно одной, возвращаем сообщение.
4. Подсчитываем количество гласных в каждой фразе с помощью генератора списков и функции sum.
5. Сравниваем количество гласных во всех фразах: если все равны, выводим "Парам пам-пам", иначе — "Пам парам".
...
def check_rhythm(stroka):
    ...
    # Проверяем, что фраз больше одной
    if len(phrases) <= 1:
